...
final class SpiderStormBehavior {
    ...
    SpiderStormBehavior(IgnisNbtService nbtService) {
    ...
    void onTrigger(RuntimeBlockInstance instance) {
        ...
        if (realExplosion) {
            ...
            spawnBurst(loc, finalPower);
        }

        spawnEntityPayload(def, loc, finalPower);
    }

    private void spawnEntityPayload(BlockDefinition def, Location loc, float finalPower) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                nbtService.editEntity(entity, nbt -> {
                ...
                if (targetPlayers && entity instanceof Mob mob) {
                    loc.getWorld().getPlayers().stream()
                            .min(Comparator.comparingDouble(player -> player.getLocation().distanceSquared(loc)))
    ...
    private void spawnBurst(Location center, float power) {
