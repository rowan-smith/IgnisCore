    ...
    <div className={styles.wrapper}>
      <div className={styles.header}>
        ...
        <p className={styles.hint}>
          Search <code>/ignis</code> commands — press <span className={styles.kbd}>/</span> to focus
          this field.
        </p>
      </div>
      <div className={styles.inputRow}>
        ...
        <input
          ...
          type="search"
          placeholder="e.g. reload, give, pack…"
          value={query}
          ...
          {results.length === 0 ? (
            ...
            results.map((entry) => (
              <li key={`${entry.docSlug}:${entry.command}`}>
                <Link className={styles.result} to={`${entry.docSlug}#${entry.anchor}`}>
      ...
      {!query.trim() && (
        <div className={styles.quickTags}>
          {['reload', 'give', 'pack', 'blocks', 'items', 'debug'].map((tag) => (
            <button key={tag} type="button" className={styles.tag} onClick={() => setQuery(tag)}>
              {tag}
            </button>
          ))}
        </div>
