...
const VARS_PATH = path.join(WEBSITE_ROOT, 'site-vars.json');
const CNAME_PATH = path.join(WEBSITE_ROOT, 'static', 'CNAME');
const PACKAGE_PATH = path.join(WEBSITE_ROOT, 'package.json');
...
function readPomVersion() {
  ...
  const projectMatch = xml.match(
    /<artifactId>igniscore-parent<\/artifactId>\s*\n\s*<version>([^<]+)<\/version>/,
  );
  ...
  return versionMatch && !versionMatch[1].includes('${') ? versionMatch[1].trim() : null;
}
