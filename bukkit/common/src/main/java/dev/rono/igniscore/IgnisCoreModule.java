...
import dev.rono.igniscore.service.CompositeNbtService;
...
import dev.rono.igniscore.service.PdcNbtService;
import dev.rono.igniscore.service.IgnisEffectServiceImpl;
import dev.rono.igniscore.service.ProtocolService;
import dev.rono.igniscore.integration.region.CompositeRegionService;
import dev.rono.igniscore.integration.region.WorldEditRegionService;
import dev.rono.igniscore.integration.hologram.BukkitHologramService;
import dev.rono.igniscore.integration.npc.CitizensNpcService;
...
public class IgnisCoreModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        bind(PdcNbtService.class).in(Scopes.SINGLETON);
        bind(CompositeNbtService.class).in(Scopes.SINGLETON);
        bind(IgnisNbtService.class).to(CompositeNbtService.class).in(Scopes.SINGLETON);
        bind(WorldEditRegionService.class).in(Scopes.SINGLETON);
        bind(CompositeRegionService.class).in(Scopes.SINGLETON);
        bind(IgnisRegionService.class).to(CompositeRegionService.class).in(Scopes.SINGLETON);
        bind(BukkitHologramService.class).in(Scopes.SINGLETON);
        bind(IgnisHologramService.class).to(BukkitHologramService.class).in(Scopes.SINGLETON);
        bind(CitizensNpcService.class).in(Scopes.SINGLETON);
        bind(IgnisNpcService.class).to(CitizensNpcService.class).in(Scopes.SINGLETON);
